...
class SeeAllHisOrdersTest {
    ...
    void testseeallhisorders(FxRobot robot)
    {
        ...
        robot.clickOn("#BackButton");
        robot.clickOn("#seeallyourordersbutton");

        robot.clickOn("#ShowOrder");
        assertThat(robot.lookup("#ShowOrderMessage").queryText()).hasText("Select an order!");

        robot.type(KeyCode.UP);
        robot.type(KeyCode.ENTER);
        robot.clickOn("#ShowOrder");
        assertThat(robot.window("Show Order")).isShowing();
    }
}